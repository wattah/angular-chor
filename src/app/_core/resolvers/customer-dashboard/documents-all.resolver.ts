...
export class DocumentFullResolver implements Resolve<Observable<DocumentVO[]>> {
  customerId: string;
  ...
  resolve(router: ActivatedRouteSnapshot): Observable<DocumentVO[]> {
    ...
    return this.documentService.getDocumentFullBy(this.customerId, TARGET_TYPE_DOCUMENT.REQUEST )
    .pipe(catchError(() => of([])));
  }
}
